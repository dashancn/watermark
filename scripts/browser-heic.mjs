...
await evaluate("(async()=>{for(const registration of await navigator.serviceWorker.getRegistrations())await registration.unregister();for(const key of await caches.keys())await caches.delete(key)})()", true);
...
const fixtureBase64 = (await readFile(new URL('../test/fixtures-libheif-example.heic', import.meta.url))).toString('base64');
await evaluate(`(()=>{const binary=atob('${fixtureBase64}');const bytes=Uint8Array.from(binary,c=>c.charCodeAt(0));const file=new File([bytes],'fixture.heic',{type:'image/heic'});const transfer=new DataTransfer();transfer.items.add(file);const input=document.querySelector('#fileInput');Object.defineProperty(input,'files',{configurable:true,value:transfer.files});input.dispatchEvent(new Event('change',{bubbles:true}))})()`);
await new Promise((resolve) => setTimeout(resolve, 12000));
const result = await evaluate(`(() => ({status:document.querySelector('#heicStatus').textContent,count:document.querySelector('#fileCount').textContent,name:document.querySelector('#currentName').textContent,width:document.querySelector('#preview').width,height:document.querySelector('#preview').height,app:[...performance.getEntriesByType('resource')].map(x=>x.name),errors:window.__errors||[]}))()`);
if (!result.status) console.error(JSON.stringify({ result, requests }, null, 2));
...
assert.ok((await runtimeResponse.arrayBuffer()).byteLength > 1_000_000);
